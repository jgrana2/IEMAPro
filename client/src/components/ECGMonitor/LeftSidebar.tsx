      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-border flex items-center justify-between">
            {isExpanded && (
              <h2 className="font-semibold text-foreground">Patient & Device</h2>
            )}
            <Button
              variant="ghost"
              size="sm"
            ...
            {isExpanded ? (
              <div className="p-4 space-y-6">
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-medium text-foreground">Current Patient</Label>
                  <Card>
                    <CardContent className="p-3">
                      {currentPatient ? (
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-sm">{currentPatient.name}</p>
                            <p className="text-xs text-muted-foreground">ID: {currentPatient.patientId}</p>
                          </div>
                        </div>
                      ) : (
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-medium text-foreground">BLE ECG Device</Label>
                  <Button onClick={onScanDevices} className="w-full" variant="outline">
                  ...
                  <div className="space-y-2">
                    {bleDevices.map((device: any) => (
                      <Card key={device.id} className="cursor-pointer hover:bg-accent">
                        <CardContent className="p-3">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              <div className={`w-2 h-2 rounded-full ${
                                device.isConnected ? 'bg-green-500 animate-pulse' : 'bg-gray-400'
                              }`} />
                              <div>
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-medium text-foreground">Buffer Configuration</Label>
                  <div className="space-y-2">
                    <div>
                      <Label className="text-xs text-muted-foreground">Buffer Size (samples)</Label>
                      <Input
                        type="number"
                        value={bufferSize}
                ...
                <div className="space-y-3">
                  <Label className="text-sm font-medium text-foreground">WebSocket Server</Label>
                  <div className="space-y-2">
                    <div>
